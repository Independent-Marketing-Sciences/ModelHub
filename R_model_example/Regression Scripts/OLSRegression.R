...
# Code to calculate VIFs
varsVIF = rep(NA,length(modVarFinalWithFixed))

if(length(varsVIF) > 2){
  
  # Generate list of possible constants for panel modelling, we want to keep all but one constant to avoid the dummy variable trap and have perfect multicollinearity
  constant_list <- list()
  for (i in 1:length(xsNames)){
    new_constant <- paste("ßßßconstantµ", i, "ßßß", sep = "")
    constant_list[[i]] <- new_constant
  }
  
  possibleConstNames = c("ßßßconstantßßß", "ßßßconsßßß", "ßßßconstßßß", "ßßßcßßß",
                         "ßßßseas_constantßßß", "ßßßseas_consßßß", "ßßßseas_constßßß",
                         "ßßßseas_cßßß",constant_list)
  
  costName = possibleConstNames[possibleConstNames %in% names(transVarByXsDataLong)]
  
  # Drop one constant from the costName variable to avoid perfect multicollinearity
  if (length(costName) > 1){
    costName = costName[-length(costName)]
  }
  
  combined_string <- paste(costName, collapse = " [+] ")
  
  modelFormWithDefaultConstText = gsub(" [+]0", "", modelFormText) %>%
    gsub(combined_string, "", .) %>%
    gsub("  [+]","",.)
  
  modelFormWithDefaultConst =  as.formula(modelFormWithDefaultConstText)
  regDataForVIF = transVarByXsDataLong[,!names(transVarByXsDataLong) %in%
                                         costName]
  
  modelFitWithDefConst <- lm(modelFormWithDefaultConst, data = regDataForVIF)
  varsVIFTemp = vif(modelFitWithDefConst)
  
  # Change names in named vector to match variables without constraints
  names(varsVIFTemp) = modVarByXsNames$varCodeWithXs[match(names(varsVIFTemp),
                                                           modVarByXsNames$varNamesNoOperator)]
  
  # Replace value only for the matched variable (to take coefficient constraints
  # into account)
  varsVIF[match(names(varsVIFTemp), modVarFinalWithFixed)] = varsVIFTemp
}

names(varsVIF) = modelledVarNames

rm(possibleConstNames, costName, modelFormWithDefaultConstText,
   modelFormWithDefaultConst, regDataForVIF, modelFitWithDefConst, varsVIFTemp)
