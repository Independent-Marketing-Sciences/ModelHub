...
if ((toplineDetailsCsv[toplineDetailsCsv$Metric == "blank dates to generate?", "Detail"] %>% tolower()) != "no") {
  ...
  # old code - only enables one date
  # blankDate <- toplineDetailsCsv[toplineDetailsCsv$Metric == "blank dates to generate?", "Detail"] %>% as.numeric %>% as.Date(origin = "1899-12-30")

  blankDate <- toplineDetailsCsv[toplineDetailsCsv$Metric == "blank dates to generate?", "Detail"] %>%
  # "24/10/2021,23/01/2022" %>%
    strsplit(",") %>%
    unlist() %>%
    as.Date(dates, format = c("%d/%m/%Y"))
  
    media.blank.4.list <- lapply(1:length(blankDate), function(v)
    data.frame(KPI.Name = rep(xsNames, each = nrow(mediaDetailsCsv)),
               OBS = rep(blankDate[v], # this is the substitute out bit
                         nrow(mediaDetailsCsv) * length(xsNames)), 
               PreNestCont = rep(TRUE, nrow(mediaDetailsCsv) * length(xsNames)),
               PostNestCont = rep(TRUE, nrow(mediaDetailsCsv) * length(xsNames)),
               variable = rep(mediaDetailsCsv$Media.Name, length(xsNames)),
  ...
  media.blank.4 <- media.blank.4.list %>%
    do.call(rbind, .)
  
  # Merge with existing split contributions
  # splitConts <- rbind(splitConts, media.blank, media.blank.2, media.blank.3, media.blank.4)
  blankConts <- rbind(media.blank, media.blank.2, media.blank.3, media.blank.4)
  
  rm(media.blank.4.list, media.blank.4)
  
} else {
  
  # Merge with existing split contributions
  # splitConts <- rbind(splitConts, media.blank, media.blank.2, media.blank.3)
  blankConts <- rbind(media.blank, media.blank.2, media.blank.3)
  
}

# Remove blank variables that already exist in the splitConts - otherwise mucks up the nesting
newBlankConts <- rbind(cbind(splitConts, TempFlag = rep(1, nrow(splitConts))), 
              cbind(blankConts, TempFlag = rep(0, nrow(blankConts))))
newBlankConts2 <- distinct(newBlankConts, KPI.Name, OBS, PreNestCont, PostNestCont, variable, PreMedSpCont, PostMedSpCont, .keep_all= TRUE)
newBlankConts3 <- newBlankConts2[newBlankConts2$TempFlag == 0, 1:8] # dropping the Temp Flag column (9)

# Merge with Split Conts database
splitConts <- rbind(splitConts, newBlankConts3)

rm(blankConts, newBlankConts, newBlankConts2, newBlankConts3)
